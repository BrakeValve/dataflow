...
for appid in ids :
    ...
    if response.status_code == 200:
        ...
        if(rows[3].string!='Downloadable Content'):
            ...
            out = out + rows[6].string + "|" + rows[7].string + "\n"
            if(rows[18].string == 'metacritic_score'):
                # metacritic_score
                app_dict[rows[18].string] = rows[19].string;
                out = out + rows[18].string + "|" + rows[19].string + "\n"
            ...
            f = open('app_meta_'+appid+'.txt','w')
            f.write(''.join(out));
            f.close();
            
             
            f = open('app_meta_'+appid+'.json','w')
